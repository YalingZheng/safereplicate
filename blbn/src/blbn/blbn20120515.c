 ...
 * Modified by Yaling Zheng on May 15th, 2012
 * to deal with comparing results with controlled experiments
 * we have two networks: Bayesian and naive
 * and we have naive_ChoiceBayesian compare with Bayesian and naive
 * and we have Bayesian_Choicenaive compare with Bayesian and naive
 * and we save all those results, so we do have 4 sets of results
 */
 ...
 */
blbn_state_t* blbn_init_state (char* type_net, char* choice_type_net, char *experiment_name, char *data_filepath, char *test_data_filepath, char *model_filepath, char *target_node_name, unsigned int budget, char *output_folder, char* policy, int k, int f) {
	...
	model_filepath_fullname = strcat(model_filepath, type_net);
	// get the correct model name (we name it with suffix .naive and .normal)
	if (type_net=="Bayesian"){
	  model_filepath_fullname = strcat(model_filepath, "normal");}
	...
	// Create log files
	sprintf (graph_filename, "%s/%s.choice.%s.graph.csv.%d", output_folder, type_net, choice_type_net, f);
	graph_fp = fopen (graph_filename, "w");

	sprintf (log_filename,   "%s/%s.choice.%s.log.txt.%d", output_folder, type_net, choice_type_net, f);
	...
	if (net != NULL) {
		...
		if (state != NULL) {
			...
			if (strstr(policy, "MB") !=NULL){
				//printf("\nMarkov Blanket here\n");
				// only consider those nodes in Markov Blanket
...
void blbn_restore_prior_network (blbn_state_t *state) {
	...
	} else {printf("state is null!!.........\n");}
	//printf("end of blbn_restore_prior_network!..........\n");
}
...
void blbn_revise_by_case_findings_v2 (blbn_state_t *state, int case_index) {
	...
	//printf("begin of blbn_revise_by_case_findings_v2\n");
	...
	*/
	...
	if (blbn_has_findings_available_not_learned (state, case_index)) {
		...
		// Unlearn case with previously-known values
		if (blbn_has_findings_learned (state, case_index)) {
			//printf("second true of blbn_has_findings_learned!\n");
		...
		//if (blbn_has_findings_not_learned (state, case_index)) {
		if (blbn_has_findings_available_not_learned (state, case_index)) {
			//printf("true here 444444444444!\n");
			blbn_learn_case_v2 (state, case_index);

		}
	}
	//printf("end of blbn_revise_by_case_findings_v2\n");
...
void blbn_learn_case_v2 (blbn_state_t *state, int case_index) {
	...
	//printf("END of blbn_learn_case_v2!\n");
...
void deep_copy_state(blbn_state_t *dest_state, blbn_state_t *origin_state){
...
void blbn_learn2(blbn_state_t *state, int policy, FILE* graph_fp, FILE* log_fp, blbn_select_action_t* curr_action) {
	//printf("calling blbn_learn2\n");
	int i;
	blbn_select_action_t *prev_action = NULL;
	blbn_select_action_t *curr_action = NULL;
	int minimum_cost;
...
int blbn_get_d_separated_nodes (blbn_state_t *state, unsigned int node_index, int **d_separated_node_indices) {
	...
	//printf("BEGIN OF blbn_get_d_separated_nodes!\n");
...
//		printf ("%s ", blbn_get_node_name (state, d_separated_node_index));
		printf ("%d ", d_separated_node_index);
	}
	printf ("}\n");
*/
	...
	//printf("END OF blbn_get_d_separated_nodes!\n");
...
int blbn_get_d_separated_node_count (blbn_state_t *state, unsigned int node_index) {
	...
	//printf("BEGIN OF blbn_get_d_separated_nodes!\n");
	/* size (return this), array_of_indices */
	//printf("Calling count number of d-separations \n");
...
/**
 * Returns an array of node indexes
 * whose representing nodes are in Markov Blanket of the target node
 *
int blbn_get_markov_blanket (blbn_state_t *state, unsigned int node_index, int** ){

	const nodelist_bn *MB_node_candidates = DupNodeList_bn(GetNetNodes_bn (state->work_net));

	node_bn *MB_node = NthNode_bn (MB_node_candidates, node_index); // Get specified node from static node list

	//GetRelatedNodes_bn (MB_nodes, "markov_boundary", MB_node);
	int num_mb_nodes = 0;

	for (i = 0; i < state->node_count; ++i) {
		// if current node is not target
		if (i!=state->target){
			if (GetRelatedNodes_bn (MB_Node_candidates[i], "markov_boundary", MB_node)){
				printf(i+" ");
				markov_blanket[num_mb_nodes] = i;
				num_mb_nodes++;
			}
...
/*int blbn_get_d_separated_nodes (blbn_state_t *state,  size (return this), array_of_indices
		unsigned int node_index, int *d_separated_node_indices) {
	...
	// Get a list of all of the node in the network
	nodelist_bn *d_separated = GetNetNodes_bn (state->work_net); // Initialize node list with all nodes in the network
	//nodelist_bn *d_separated = NewNodeList2_bn(8, state->work_net);

	// Get the node with the specified node index
	node_bn *d_separating_node = NthNode_bn (d_separated, node_index); // Get specified node from static node list

	// Find all the nodes that are d-connected with node, subtract them from the
	// specified node list, and exclude the specified node from the node list.
	// The remaining nodes will be those that are d-separated from the
	// specified node.
	GetRelatedNodes_bn (d_separated, "d_connected,subtract", d_separating_node);
	// Subtract node of interest and all nodes in the relation (d-connected) to the specified node of interest

	// NOTE: At this point, the node list d_separated contains the nodes d-separated from the specified node of interest

	// TODO: Remove the nodes with set/specified findings from list of nodes
		// TODO: (?) Iterate through nodes, check if each has a finding specified, if so, remove it from the list of d-separated nodes (since it is in the separating set)

	// Create an array with indices of d-separated nodes
	// Iterate through nodes and get indices in static ordering of each of them
	//printf ("node idx at %d\n", blbn_get_node_index(state, "Cancer"));
	printf ("DEBUG: d-separations for node %s: { ", blbn_get_node_name (state, node_index));
	int d_separated_node_count = LengthNodeList_bn (d_separated);
	d_separated_node_indices = (int *) malloc (sizeof (int) * d_separated_node_count); // Create array to hold d-separated node indices
	for (i = 0; i < d_separated_node_count; ++i) {
		int current_node_index = blbn_get_node_index (state, GetNodeName_bn (NthNode_bn (d_separated, i)));
		//printf ("%d ", current_node_index);
		d_separated_node_indices[i] = current_node_index; // Get index (from static ordering) corresponding to the current node at index i in the d-separated node list
		printf ("%s ", blbn_get_node_name (state, current_node_index));
	}
	printf ("}\n");
...
blbn_select_action_t* blbn_select_next_empgdsep (blbn_state_t *state) {
	//printf("calling select_next_empgdsep");
	...
	//printf("Yes, it is blbn_select_next_empgdsep   .....        \n");
	...
	if (curr_action != NULL) {
		...
		*/
		...
		/*
		printf("[ ");
		//for (j = 0; j < state->node_count; j++) {
		for (j=0; j<state->nodes_consider[0];j++){
			//[ -1 -2 1 0 -1 -2 0 -2 ]
			printf("%d ", dsep_values[j][0]);
		...
		for (j = 0; j < state->case_count; ++j) {
			...
			//for (i = 0; i < state->node_count; ++i) {
			int ii=0;
			//for (ii=0; ii<state->node_count;ii++){
			for (ii=0; ii<state->nodes_consider[0];ii++){
				i = state->nodes_consider[1+ii];
				// Check scores for values that are not for the target node or nodes that are already purchased
				if (!blbn_is_available_finding (state, i, j)) {
	...
	//printf("end of calling select_next_empgdsep");

	//printf("END OF CALLING blbn_select_next_empgdsep   .....        \n");
...
blbn_select_action_t* blbn_select_next_empgdsepw1 (blbn_state_t *state) {
	...
	//printf("Yes, it is blbn_select_next_empgdsepw1   .....        \n");
	...
	//printf("END OF CALLING blbn_select_next_empgdsepw1   .....        \n");
...
blbn_select_action_t* blbn_select_next_empgdsepw2 (blbn_state_t *state) {
	...
	if (curr_action != NULL) {
		...
		*/
		...
		/*
		for (ii=0; ii<state->nodes_consider[0];ii++){
					printf("%f ", gain_values[ii]);
					printf("%d ", dsep_values[ii]);
				}
				*/
		for (ii=0; ii<state->nodes_consider[0];ii++){
			free (gain_values[ii]);
	...
	//printf("END OF CALLING blbn_select_next_empgdsepw2   .....        \n");
...
int** blbn_util_dsep (blbn_state_t *state) {
	...
	//printf("end of calling blbn_util_dsep\n");
	return dsep_values;
}
